...
df_samp$posttext <- gsub(" i ", " iii ", df_samp$posttext)
df_samp$posttext <- gsub(" me ", " mee ", df_samp$posttext)
df_samp$posttext <- gsub(" my ", " myy ", df_samp$posttext)
df_samp$posttext <- gsub("  ", " ", df_samp$posttext)
...
write.csv(ldaOut_s.topics,file=paste("LDAGibbs_s",k,iter,"DocsToTopics_s.csv"))
...
ap_gamma <- tidy(ldaOut_s, matrix = "gamma")
ap_gamma
...
gammaldaOut_s = as.data.frame(ldaOut_s@gamma)

sVem = LDA(dtm, 30, method="VEM")

#extract the most likely terms for each topic (top 10 words)
sTerms = terms(sVem, 10)

#extract the most likely topics for each document
sTopics = topics(sVem, 5)

#make a dataframe with the probabilities for each document in each topic
gammaSVem = as.data.frame(sVem@gamma)
...
df_s$bname <- NA
df_s$bname[grep("71 Toes",df_s$fname)] <- "71 Toes"
df_s$bname[grep("Dooce",df_s$fname)] <- "Dooce"
df_s$bname[grep("Amalah",df_s$fname)] <- "Amalah"
df_s$bname[grep("Sycamore",df_s$fname)] <- "Sycamore"
df_s$bname[grep("Nie Nie",df_s$fname)] <- "Nie Nie"
